{
  ...
  outputs = { self, nixpkgs, disko, ... }: {
    ...
    packages.default.install = nixpkgs.lib.genAttrs [ "x86_64-linux" "aarch64-linux" ] (arch: nixpkgs.legacyPackages.${arch}.writeScriptBin "install" ''
      ...
      sudo nixos-generate-config --fo-filesystems --show-hardware-config > hardware-configuration.nix
      git add hardware-configuration.nix
      sudo nix --extra-experimental-features 'nix-command flakes' run github:nix-community/disko#disk-install -- --flake .#streamdesk --disk main /dev/nvme0n1
    '');
  };
}
